...
import { FormField } from "../components/FormField"; // Ensure this component supports the necessary input types
...
export const SchoolFormDemo = () => {
  ...
  const handleChange = (e) => {
    ...
    switch (name) {
      ...
      case "email":
        const emailValid = value.trim() !== "" && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(value);
        setError((prev) => ({ ...prev, email: emailValid ? "" : "Email is invalid" }));
        setForm((prev) => ({ ...prev, [name]: value }));
        break;
    ...
    if (validateRequiredFields()) {
      ...
      emailjs
        .send(
          ...
          { schoolName: form.schoolName, personName: form.personName, email: form.email, message: msg },
          import.meta.env.VITE_PUBLIC_KEY
        )
        .then(() => {
          setGlobalMessage("Thanks for contacting us, we will get back to you soon!");
          setForm(initialVal);
          setTimeout(() => {
            setGlobalMessage("");
          }, 5000);
