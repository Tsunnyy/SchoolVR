...
export const ColabFormDemo = () => {
  ...
  const handleChange = (e) => {
    ...
    switch (name) {
      ...
      case "fullname":
        value.trim() === ""
          ? setError((prevError) => ({ ...prevError, fullname: "Full Name is required" }))
      ...
      case "website":
        value.trim() === ""
          ? setError((prevError) => ({ ...prevError, website: "Website is required" }))
          : setError((prevError) => ({ ...prevError, website: "" }));
        setForm((prevForm) => ({ ...prevForm, [name]: value }));
        break;

      case "otherPartnershipType":
        if (value.trim().length < 10 || value.trim().length > 50) {
          setError((prevError) => ({
            ...prevError,
            otherPartnershipType: "Other Partnership Type must be between 10 and 50 characters",
          }));
        } else {
          setError((prevError) => ({ ...prevError, otherPartnershipType: "" }));
        }
        setForm((prevForm) => ({ ...prevForm, [name]: value }));
        break;
      case "designation":
  ...
  const validateRequiredFields = () => {
    ...
    if (form.partnershipType === "Other" && !form.otherPartnershipType.trim()) {
      newErrorState.otherPartnershipType = "Please specify the type of partnership";
      isValid = false;
    } else {
      newErrorState.otherPartnershipType = "";
    }
  ...
  const handleSelectChange = (selectedOption) => {
    if (selectedOption) {
      setForm((prevForm) => ({
        ...prevForm,
        partnershipType: selectedOption.label,
        otherPartnershipType: selectedOption.label === "Other" ? "" : prevForm.otherPartnershipType
      }));
    ...
    <form action="" className="grid-2" onSubmit={handleSubmit}>
      ...
      <div className="newSelectSection">
        ...
        <Select
          options={options}
          placeholder="Country"
          // onChange={(selectedOption) => setForm((prev) => ({ ...prev, country: selectedOption.value }))}
      ...
      <div className="newSelectSection">
        ...
        <Select
          ...
          placeholder="Partnership/Collaboration"
          className="custom-select-dropdown"
          classNamePrefix={"custom-select-dropdown-inner"}
      ...
      {form.partnershipType === "Other" && (
        <FormField
          ...
          onChange={handleChange}
          errorMessage={error.otherPartnershipType}
        />
      )}
      <FormField
        type="textarea"
