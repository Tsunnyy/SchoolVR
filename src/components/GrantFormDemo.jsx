...
import { FormField } from "../components/FormField"; // Make sure this component supports the necessary input types
...
export const GrantFormDemo = () => {
    ...
    switch (name) {
      case "schoolName":
        if (value.trim().length < 10) {
          setError((prev) => ({ ...prev, schoolName: "School Name must be at least 10 characters" }));
        } else {
      ...
      case "designation":
        value.trim() === ""
          ? setError((prev) => ({ ...prev, [name]: `${name.replace(/([A-Z])/g, ' $1').trim()} is required` }))
      ...
      case "message":
        if (value.length < 30 || value.length > 500) {
          setError((prev) => ({ ...prev, message: "Message must be between 30 and 500 characters" }));
        } else {
      ...
      case "website":
        // Optional field, can add validations if needed
        setForm((prev) => ({ ...prev, [name]: value }));
        break;
  ...
  const validateRequiredFields = () => {
    const requiredFields = ["schoolName", "personName", "designation", "country", "message"];
    let isValid = true;
    let newErrorState = {};

    requiredFields.forEach((field) => {
      if (!form[field].trim()) {
        newErrorState[field] = `${field.replace(/([A-Z])/g, ' $1').trim()} is required`;
        isValid = false;
      } else {
        newErrorState[field] = "";
      }
  ...
  const handleSubmit = (e) => {
    ...
    if (validateRequiredFields()) {
      const msg = `Website: ${form.website}\nMessage: ${form.message}`;
      emailjs
        .send(
          import.meta.env.VITE_SERVICE_ID,
          import.meta.env.VITE_TEMPLATE_ID,
          { schoolName: form.schoolName, personName: form.personName, message: msg },
          import.meta.env.VITE_PUBLIC_KEY
        )
        .then(() => {
          setGlobalMessage("Thank you for your application! We will get back to you soon.");
          setForm(initialVal);
          setTimeout(() => {
            setGlobalMessage("");
          }, 5000);
  ...
  const options = countryListAllIsoData.map(country => ({
    value: country.code,
    label: country.name,
  }));
  ...
  return (
    <form action="" className="grid-2" onSubmit={handleSubmit}>
      ...
      <FormField
        type="textarea"
        placeholder="Message* (Why your institution should be selected for this funding, and your institution's commitment to advancing STEM education)"
        name="message"
        className="col-2"
        value={form.message}
        onChange={handleChange}
